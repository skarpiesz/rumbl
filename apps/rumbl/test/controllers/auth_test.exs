defmodule Rumbl.AuthTest do
  ...
  test "authenticate_user continues when the current_user exists", %{conn: conn} do
    conn = conn |> assign(:current_user, %Rumbl.User{}) |> Auth.authenticate_user([])
    refute conn.halted
  end

  test "login puts the user in the session", %{conn: conn} do
    login_conn = conn |> Auth.login(%Rumbl.User{id: 123}) |> send_resp(:ok, "")
  ...
  test "logout drops the session", %{conn: conn} do
    logout_conn = conn |> put_session(:user_id, 123) |> Auth.logout |> send_resp(:ok, "")
  ...
  test "call places user from session into assigns", %{conn: conn} do
    user = insert_user
    conn = conn |> put_session(:user_id, user.id) |> Auth.call(Repo)
